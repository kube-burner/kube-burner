...
func (ex *JobExecutor) gc(ctx context.Context, wg *sync.WaitGroup) {
	...
	for _, obj := range ex.objects {
		...
		for _, gvr := range gvrsToCleanup {
			ex.limiter.Wait(ctx)
			if !obj.namespaced {
				cleanupNonNamespacedResourcesByGVR(ctx, *ex, gvr, labelSelector)
			} else if obj.namespace != "" { // When the object has a fixed namespace not generated by kube-burner
				cleanupNamespaceResourcesByGVR(ctx, *ex, gvr, obj.namespace, labelSelector)
				err := waitForDeleteResourceByGVRInNamespace(ctx, *ex, gvr, obj.namespace, labelSelector)
				if err != nil {
					log.Fatal(err.Error())
				}
			}
