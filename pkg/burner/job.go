...
func Run(configSpec config.Spec, kubeClientProvider *config.KubeClientProvider, metricsScraper metrics.Scraper, additionalMeasurementFactoryMap map[string]measurements.NewMeasurementFactory, embedCfg *fileutils.EmbedConfiguration) (int, error) {
	return RunWithOutput(configSpec, kubeClientProvider, metricsScraper, additionalMeasurementFactoryMap, embedCfg, "", "")
}

// RunWithOutput runs the benchmark with output format options
//
//nolint:gocyclo
func RunWithOutput(configSpec config.Spec, kubeClientProvider *config.KubeClientProvider, metricsScraper metrics.Scraper, additionalMeasurementFactoryMap map[string]measurements.NewMeasurementFactory, embedCfg *fileutils.EmbedConfiguration, outputFormat, outputFile string) (int, error) {
	var err error
	var rc int
...
func generateAndOutputSummary(uuid string, executedJobs []prometheus.Job, returnMap map[string]returnPair, metricsScraper metrics.Scraper, outputFormat, outputFile string, returnCode int, executionErrors string) {
	...
	for i, job := range executedJobs {
		if i == 0 || job.Start.Before(startTime) {
			startTime = job.Start
		}
		if i == 0 || job.End.After(endTime) {
			endTime = job.End
		}

		passed := true
		jobErrors := ""
		if value, exists := returnMap[job.JobConfig.Name]; exists {
			passed = value.innerRC == 0
			jobErrors = value.executionErrors
		}
		...
		elapsedTime := job.End.Sub(job.Start).Round(time.Second).Seconds()
		if elapsedTime > 0 {
			achievedQps = float64(job.ObjectOperations) / elapsedTime
		}
	...
	summary := BenchmarkSummary{
		...
		TotalJobs:       len(executedJobs),
		PassedJobs:      passedJobs,
		FailedJobs:      len(executedJobs) - passedJobs,
	...
	if outputFile != "" {
		err = OutputSummaryToFile(summary, outputFile, format)
		if err != nil {
			log.Errorf("Failed to write summary to file %s: %v", outputFile, err)
		} else {
			log.Infof("Summary written to %s", outputFile)
		}
	} else {
		err = OutputSummary(summary, format)
		if err != nil {
			log.Errorf("Failed to output summary: %v", err)
		}
	}
}
