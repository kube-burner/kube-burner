...
func Run(configSpec config.Spec, kubeClientProvider *config.KubeClientProvider, metricsScraper metrics.Scraper, additionalMeasurementFactoryMap map[string]measurements.NewMeasurementFactory) (int, error) {
	...
	go func() {
		...
		jobExecutors = newExecutorList(configSpec, kubeClientProvider)
		...
		for jobExecutorIdx, jobExecutor := range jobExecutors {
			...
			if measurementsInstance == nil {
				...
				measurementsInstance = measurementsFactory.NewMeasurements(&jobExecutor.Job, kubeClientProvider)
			...
			if jobExecutor.BeforeCleanup != "" {
				...
				stdOut, stdErr, err := util.RunShellCmd(jobExecutor.BeforeCleanup, configSpec.EmbedFS, configSpec.EmbedFSDir)
				if err != nil {
			...
			jobEnd := time.Now().UTC()
			if jobExecutor.MetricsClosing == "afterJob" {
			...
			if jobExecutor.MetricsClosing == "afterJobPause" {
			...
			if !jobExecutor.MetricsAggregate {
				...
				if jobExecutor.MetricsClosing == "afterMeasurements" {
					executedJobs[len(executedJobs)-1].End = time.Now().UTC()
				}
...
func newExecutorList(configSpec config.Spec, kubeClientProvider *config.KubeClientProvider) []JobExecutor {
