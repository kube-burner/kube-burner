...
func runLifecycleHook(name string, script string, background bool, configSpec config.Spec, errs *[]error, rc *int) {
	...
	logMsg := fmt.Sprintf("Running %s command %s", name, script)
	if !background {
		logMsg = fmt.Sprintf("Waiting for %s command %s to finish", name, script)
	}
	log.Info(logMsg)

	stdOut, stdErr, err := util.RunShellCmd(script, configSpec.EmbedFS, configSpec.EmbedFSDir, background)
	if err != nil {
	...
	log.Infof("%s out: %v, err: %v", name, stdOut.String(), stdErr.String())
}
