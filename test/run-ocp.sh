...
/tmp/kube-burner ocp node-density --pods-per-node=50 --pod-ready-threshold=10s --container-image=gcr.io/google_containers/pause:3.0 ${COMMON_FLAGS}
oc delete ns -l kube-burner-uuid=${UUID}
popd
echo "Running node-density-heavy wrapper"
pushd ../config/node-density-heavy
/tmp/kube-burner ocp node-density-heavy --pods-per-node=40 ${COMMON_FLAGS}
oc delete ns -l kube-burner-uuid=${UUID}
popd
echo "Running cluster-density wrapper"
pushd ../config/cluster-density
/tmp/kube-burner ocp node-density-heavy --iterations=3 ${COMMON_FLAGS}
oc delete ns -l kube-burner-uuid=${UUID} 
popd
