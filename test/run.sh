...
timeout 500 kube-burner init -c kube-burner.yml --uuid ${uuid} --loglevel=debug -u http://localhost:9090 -m metrics-profile.yaml -a alert-profile.yaml
check_files
...
timeout 500 kube-burner init -c kube-burner-delete.yml --uuid ${uuid} --loglevel=debug
