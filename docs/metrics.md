...
```
And the metrics-endpoints.yaml file would look something like this.
```
...
- endpoint: http://remotehost:9090
  token: <token>
  profile: metrics-ovn.yaml
  start: 132413423
  end: 234343123
```
Note: Options `profile`, `start` and `end` are optional. If not provided will be taken from the CLI flags first or else will be populated with the default values.